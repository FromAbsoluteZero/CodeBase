# For this linear model, class weighting barely
# moves the ranking and destroys calibration. A tree
# ensemble can behave differently -- see the text.
plain = make_pipeline(StandardScaler(),
                      LogisticRegression(max_iter=5000)).fit(Xtr, ytr)
bal = make_pipeline(StandardScaler(),
                    LogisticRegression(max_iter=5000,
                                       class_weight="balanced")).fit(Xtr,
                                                                     ytr)

print(f"{'model':<12}{'AUC':>9}{'avg prec':>11}{'mean pred':>12}"
      f"{'actual':>10}")
