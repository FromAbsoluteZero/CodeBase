...
def simulate_stopping(threshold, true_step_needed, max_steps,
                      n_trials, seed):
    ...
    for _ in range(n_trials):
        ...
        for step in range(1, max_steps + 1):
            # confidence rises noisily as the
            # agent approaches the true answer
...
print(f"{'threshold':>11}{'premature-stop rate':>21}"
      f"{'avg wasted steps':>18}")
for threshold in (0.5, 0.7, 0.85, 0.95, 0.99):
    premature, wasted = simulate_stopping(threshold, true_step_needed=8,
                                          max_steps=20,
                                          n_trials=10000, seed=41)
    print(f"{threshold:>11.2f}{premature:>21.4f}{wasted:>18.3f}")

print(f"\na low threshold stops early, often before the task is "
      f"actually done;")
