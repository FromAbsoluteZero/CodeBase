...
def train_lora(Xtr, Htr_target, Xte, Hte_target, W_frozen, rank, seed,
               epochs=300, eta=1.0):
    rr = np.random.default_rng(seed)
    # small init, as the LoRA paper prescribes
    A = rr.normal(0, 0.01, (D, rank))
...
print(f"{'rank':>6}{'trainable params':>18}{'% of full':>12}"
      f"{'held-out MSE':>14}")
for rank in (1, 2, 4, 8, 16, 32):
    n_params = D * rank * 2
    _, _, loss = train_lora(Xtr, Htr_target, Xte, Hte_target, W_pretrained,
                            rank=rank, seed=39)
    if rank == 4: lora4_loss = loss           # carried forward to Step 5
    print(f"{rank:>6}{n_params:>18,}"
          f"{100*n_params/full_params:>12.2f}{loss:>14.6f}")

print(f"\nfor reference: unadapted {loss_frozen:.4f}, full fine-tune "
      f"{loss_full:.6f} "
