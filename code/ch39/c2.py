...
# what the target task actually wants
Htr_target = np.tanh(Xtr @ W_target_true)
...
def train_full(Xtr, Htr_target, Xte, Hte_target, W_init,
               epochs=300, eta=1.0):
    W = W_init.copy()
...
W_full, loss_full = train_full(Xtr, Htr_target, Xte, Hte_target,
                               W_pretrained)

print(f"held-out MSE, unadapted pretrained matrix: {loss_frozen:.4f}")
print(f"held-out MSE, after FULL fine-tuning ({full_params:,} params): "
      f"{loss_full:.6f}")
