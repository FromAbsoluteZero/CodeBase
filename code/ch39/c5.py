...
def train_adapter(Xtr, Htr_target, Xte, Hte_target, W_frozen,
                  bottleneck, seed,
                  epochs=500, eta=0.01):
...
for bn in (2, 4, 8, 16):
    loss, n_params = train_adapter(Xtr, Htr_target, Xte, Hte_target,
                                   W_pretrained,
                                   bottleneck=bn, seed=39)
    print(f"{bn:>11}{n_params:>18,}{loss:>14.6f}")

print(f"\nfor reference: LoRA at rank 4 used 2,048 params for MSE "
      f"{lora4_loss:.6f}")
print(f"unadapted {loss_frozen:.4f}, full fine-tune {loss_full:.6f} "
      f"({full_params:,} params)")
print(f"\nthe gap is structural, not a fair ranking of the "
      f"two methods: this")
