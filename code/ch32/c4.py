...
# exactly as Chapter 10 prescribed. This is the longest hand-derived
# gradient so far, and it is noticeably more work than any before
...
# a stand-in upstream gradient
dout = np.random.default_rng(32).normal(size=feat.shape)
...
for (di, dj) in [(0, 0), (1, 1), (2, 2)]:
    ...
    print(f"kernel{(di,dj)}{dkernel[di,dj]:>12.6f}{numeric:>12.6f}"
          f"{str(match):>8}")
...
for (pi, pj) in [(1, 1), (4, 4), (6, 6)]:
    ...
    print(f"pixel{(pi,pj)} {dimg[pi,pj]:>12.6f}{numeric:>12.6f}"
          f"{str(match):>8}")
