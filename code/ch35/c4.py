...
print(f"{'target examples':>17}{'frozen transfer':>18}{'from scratch':>14}"
      f"{'fine-tuned':>12}")
...
for n_per_class in (2, 5, 15, 40):
    ...
    for c in range(5):
        ...
        idx.extend(rng_sub2.choice(class_idx, size=min(n_per_class,
                                                       len(class_idx)),
                                   replace=False))
    idx = np.array(idx)
    ...
    print(f"{len(idx):>17}{acc_frozen:>18.4f}{acc_scratch:>14.4f}"
          f"{acc_finetune:>12.4f}")
