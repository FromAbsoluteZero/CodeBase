...
feat_te_embed = extract_features(Xtgt_te, src_filters)
...
print(f"\nk-NN accuracy directly in the transferred embedding "
      f"space: {knn_acc:.4f}")
...
knn_raw = KNeighborsClassifier(5).fit(Xtgt_tr.reshape(len(Xtgt_tr),
                                                      -1), ytgt_tr)
knn_raw_acc = knn_raw.score(Xtgt_te.reshape(len(Xtgt_te), -1), ytgt_te)
print(f"k-NN accuracy on raw pixels, no embedding at all:      "
      f"{knn_raw_acc:.4f}")
